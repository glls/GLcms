...
class m140510_194204_content extends CDbMigration
{
    public function safeUp()
    {
        ...
        $this->createTable('content_metadata', [
            'id'         => 'pk',
            'content_id' => 'integer',
            'key'        => 'string not null',
        ...
        $this->addForeignKey('content_categories', 'content', 'category_id', 'categories', 'id', null, 'CASCADE',
            'CASCADE');

        $this->createIndex('content_slug', 'content', 'slug', true);
        $this->createIndex('categories_slug', 'categories', 'slug', true);

        $this->addForeignKey('content_metadata', 'content_metadata', 'content_id', 'content', 'id', null, 'CASCADE',
            'CASCADE');
    }
