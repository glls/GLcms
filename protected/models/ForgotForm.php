...
class ForgotForm extends CFormModel
{
    ...
    {
        $this->_user = User::model()->findByAttributes(['email' => $this->email]);
    ...
    public function save()
    {
        ...
        if ($this->_user->save()) {
            ...
            $email->setFrom(Yii::app()->params['includes']['sendgrid']['from'])
                ...
                ->setHtml(Yii::app()->controller->renderPartial('//email/forgot', ['user' => $this->_user], true));
            ...
            $this->addError('email',
                'Unable to send reset link. This is likely a temporary error. Please try again in a few minutes.');
        }
